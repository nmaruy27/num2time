...
type formatType = 'hms' | 'hm' | 'h'

function zeropad(number: number): string {
  return String(number).padStart(2, '0');
}

function convertFormat(hh:string, mm: string, ss: string, format: formatType, sep: string): string {
  if(format == 'hms') {
...
export default function num2time(number: number, format: formatType = 'hms', sep = ':'): NttReturnType {
  if(number < 60) {
